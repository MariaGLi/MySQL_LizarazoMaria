...
-- Tabla de inserciones de experimentos.
...
-- Funcion que calcule el IVA de los productos.
DELIMITER //
...
Select nombre, totalIVA(precio, 0.19) as Precio_Con_IVA from productos;

-- Eliminar una función existente.
-- drop function IF exists totalIVA;

-- Funcion que calcule el IVA de los productos (NO DETERMINISTICO).
DELIMITER //
create function total_IVA_RAND (precio DECIMAL(10,2))
...
-- Uso de función creada.
Select nombre, total_IVA_RAND(precio, 0.19) as Precio_Con_IVA from productos;
...
SELECT ingreso_promedio_cliente (5);
...
INSERT INTO ordenes2 (cliente_id, precio, categoria_cliente) VALUES
(1, 120.99, 'Normal'),
(2, 540.50, 'Premium'),
(3, 320.75, 'Normal'),
(4, 15.99, 'Normal'),
...
DELIMITER //
Create function ingreso_promedio_cliente2 (cliente_id_interno INT)
returns decimal(10,2)
deterministic
begin
	declare ingreso decimal (10,2);
    select avg(precio) into ingreso from ordenes2 where cliente_id_interno = cliente_id; -- AVERAGE
    return ingreso;
end //
DELIMITER ;

select ingreso_promedio_cliente2 (10);